    ...
    <div className="m-auto lg:w-8/12 md:w-9/12 w-12/12">
      ...
      <ul className="border-thin">
        {items.map(item => <li key={item.title} className="flex border-thin-gray-bottom">
            <p className="capitalize py-4 slight-white-bg md:text-md text-sm basis-4/12 md:basis-3/12 px-4 flex justify-start items-center text-neutral-600">{item.title}</p>
            <p className="capitalize border-thin-left py-4 font-medium md:text-lg text-md basis-8/12 md:basis-9/12 px-4 flex justify-start items-center">{item.desc}</p>
        </li>)}
      </ul>
    </div>
  );
