<x-app-layout>
    ...
    <div class="px-8 mt-10 w-8/12">
        ...
        <form action="{{ route('update-fam-sport', $famOffRoad->id) }}" method="POST" class="flex flex-col">
            @csrf
            @method('PUT')
            <h1 class="font-bold text-4xl">Family Page of <span class="capitalize text-red-500">{{$famOffRoad->type}}</span></h1>
            <hr class="my-3 w-100">
                 ...
                 <h1 class="font-bold text-3xl my-2">Base Info</h1>
                    ...
                    <input type="text" id="familyPageBannerVideo" name="familyPageBannerVideo" value="{{ $famOffRoad['familyPageBannerVideo'] }}">


                    <label for="configPageLink">Config Page Link:</label>
                    <input type="text" id="configPageLink" name="configPageLink" value="{{ $famOffRoad['configPageInfo']['link'] }}">


                    <label for="configFamilyPageImageSrc">Config Family Page Image Source:</label>
                    <input type="text" id="configFamilyPageImageSrc" name="configFamilyPageImageSrc" value="{{ $famOffRoad['configFamilyPageInfo']['image']['src'] }}">


                    <label for="configFamilyPageImageAlt">Config Family Page Image Alt:</label>
                    <input type="text" id="configFamilyPageImageAlt" name="configFamilyPageImageAlt" value="{{ $famOffRoad['configFamilyPageInfo']['image']['alt'] }}">
                ...
                @foreach ($famOffRoad['grayCaro'] as $index => $caroItem)
                    <h4>Carousel Item {{ $index + 1 }}</h4>
                        ...
                        <input type="text" id="carouselItems[{{ $index }}][title]" name="carouselItems[{{ $index }}][title]" value="{{ $caroItem['title'] }}">
                        ...
                        <input type="text" id="carouselItems[{{ $index }}][image]" name="carouselItems[{{ $index }}][image]" value="{{ $caroItem['image'] }}">
                    @endforeach
