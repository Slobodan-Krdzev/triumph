<x-app-layout>
    <x-header>
        Edit of <span class="text-red-500 uppercase">{{ $moto->model }}</span> Motorcycle
    </x-header>
    <div class="px-8 mt-10 w-full">
        ...
        @endif
        <form action="{{ route('update-motorcycle', $moto->id) }}" method="POST" class="flex flex-col"
                    ...
                    @if(isset($moto->bikePageImageGallery))
                        @for($i=0; $i<count($moto->bikePageImageGallery); $i++)
                            <div class="{{ $i > 0 ? 'dynamic-input-wrapper' : '' }}">
                                ...
                                @if($i > 0 && $i === count($moto->bikePageImageGallery) - 1)
                                    <x-form.remove-field-button />
                                @endif
                            </div>
                        @endfor
