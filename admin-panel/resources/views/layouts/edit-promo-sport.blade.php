<x-app-layout>
    <div class="px-8 mt-10 w-8/12">
        ...
        <form method="post" action="{{ route('update-promo-sport', ['id' => $promo->id]) }}" class="flex flex-col">
            @csrf
            @method('PUT')
            ...
            @foreach ($promo->promo_data as $key => $promoItem)
                ...
                <input type="text" name="promo[{{ $key }}][image]"
                    ...
                    <x-form.input labelText="Image:" imageId="promoSportImage"
                    name="promo[{{ $key }}][image]">
                    <img id='promoSportImage'
