...
@if (isset($databaseData))
    ...
    @for($i=0; $i<count($databaseData); $i++)
        <div class="m-3 shadow-lg shadow-blue-500 p-8 dynamic-input-wrapper">
            <h1 class="font-bold text-3xl my-2">Accessory Type {{$i+1}}</h1>
            <x-form.text-input forId="accessory_type_{{ $i }}_title" placeholder="Accessory Type {{$i+1}} Title"
                               dataName="accessory[accessoryTypes][{{ $i }}][title]"
                               oldValue="accessory.accessoryTypes.{{ $i }}.title"
                               dataValue="{{ $databaseData[$i]['title'] ?? '' }}"
                               imageDesc="images/sub-family/accessory_type_title.png"/>

            <x-form.textarea forId="accessory_type_{{ $i }}_desc" placeholder="Accessory Type {{$i+1}} Description"
                             dataName="accessory[accessoryTypes][{{ $i }}][desc]"
                             oldValue="accessory.accessoryTypes.{{ $i }}.desc"
                             dataValue="{{ $databaseData[$i]['desc'] ?? '' }}"
                             imageDesc="images/sub-family/accessory_type_desc.png"/>

            <x-form.dynamic-text-inputs header="Accesory Type {{$i+1}} Items List"
                                        ...
                                        imageDesc1="images/sub-family/accessory_type_item_list.png"/>


            <x-form.input labelText="Accesory Type {{$i+1}} Image 1 URL:" imageId="accessoryType{{ $i }}Image1Url"
                          name="accessory[accessoryTypes][{{ $i }}][image1][src]"
                          imageDesc="images/sub-family/accessory_type_image1.png">
                <img id='accessoryType{{ $i }}Image1Url' src="{{ $databaseData[$i]['image1']['src'] ?? '' }}"
                     width="200" height="150"
                     class="object-cover border m-3 border-gray-200"/>
            </x-form.input>

            <x-form.text-input forId="accessory_type_{{ $i }}_image1_alt"
                               placeholder="Accessory Type {{$i+1}} Image 1 Alt Text"
            ...
            <x-form.input labelText="Accesory Type {{$i+1}} Image 2 URL:" imageId="accessoryType{{ $i }}Image2Url"
                          name="accessory[accessoryTypes][{{ $i }}][image2][src]"
                          imageDesc="images/sub-family/accessory_type_image2.png">
                <img id='accessoryType{{ $i }}Image2Url' src="{{ $databaseData[$i]['image2']['src'] ?? '' }}"
                     width="200" height="150"
                     class="object-cover border m-3 border-gray-200"/>
            </x-form.input>

            <x-form.text-input forId="accessory_type_{{ $i }}_image2_alt"
                               placeholder="Accessory Type {{$i+1}} Image 2 Alt Text"
    ...
    @endfor
@else
    <div class="m-3 shadow-lg shadow-blue-500 p-8">
        <h1 class="font-bold text-3xl my-2">Accessory Type 1</h1>
