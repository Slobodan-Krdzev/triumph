...
<div>
    <h1 class="font-bold text-2xl my-2">{{ $header }}</h1>
    <div class="dynamic-section" id="{{ $divId }}"
         data-id1="{{ $dataId1 }}" data-label1="{{ $dataLabel1 }}" data-imagedesc1="{{ $imageDesc1 }}"
         data-imagedesc2="{{ $imageDesc2 }}" data-imagedesc3="{{ $imageDesc3 }}" data-name="{{ $dataName }}"
            ...
            <div>
                @if($dataLabel1 == 'desc')
                    <x-form.textarea forId="{{ $dataId1 }}" placeholder="{{ ucfirst($dataLabel1) }}ription"
                                     dataName="{{ $dataName }}[0][desc]" oldValue="{{ $dataName }}.0.desc" imageDesc="{{ $imageDesc1 }}"/>
                @else
                    ...
                    <div class="flex items-center">
                        ...
                        <div class="cursor-pointer">
                            <x-form.modal id="{{ $dataId1 }}_0" name="{{ ucfirst($dataLabel1) }}"
                ...
                @elseif($dataId2 && $dataLabel2)
                    ...
                    <div class="flex items-center">
                        ...
                        <div class="cursor-pointer">
                            <x-form.modal id="{{ $dataId2 }}_0" name="{{ ucfirst($dataLabel2) }}"
                ...
                @if($imgLabel)
                    <x-form.input labelText="{{ $imgLabel }}:" imageId="{{ $dataName }}_0"
                                  name="{{ $dataName }}[0][image]" imageDesc="{{ $imageDesc3 }}">
        ...
        @else
            ...
            @for($i=0; $i<count($databaseData); $i++)
                <div class="{{ $i > 0 ? 'dynamic-input-wrapper' : '' }}">
                    @if($dataLabel1 == 'desc')
                        <x-form.textarea forId="{{ $dataId1 }}_{{ $i }}"
                                         placeholder="{{ ucfirst($dataLabel1) }}ription {{ $i+1 }}"
                    ...
                    @else
                        <label for="{{ $dataId1 }}_{{ $i }}"
                               class="block text-sm font-medium text-gray-700">{{ ucfirst($dataLabel1) }} {{ $i+1 }}
                            :</label>
                        <div class="flex items-center">
                    ...
                    @endif
                    @if($dataId2 && $dataLabel2 && $dataLabel2 == 'desc')
                        <x-form.textarea forId="{{ $dataId2 }}_{{ $i }}"
                                         placeholder="{{ ucfirst($dataLabel2) }}ription {{ $i+1 }}"
                                         dataName="{{ $dataName }}[{{ $i }}][desc]"
                                         oldValue="{{ $dataName }}.{{ $i }}.desc"
                    ...
                    @elseif($dataId2 && $dataLabel2)
                        <label for="{{ $dataId2 }}_{{ $i }}"
                               class="block text-sm font-medium text-gray-700 mt-3">{{ ucfirst($dataLabel2) }} {{ $i+1 }}
                            :</label>
                        <div class="flex items-center">
                    ...
                    @if($imgLabel)
                        <x-form.input labelText="{{ $imgLabel }}:" imageId="{{ $dataName }}[{{ $i }}][image]"
                                      name="{{ $dataName }}[{{ $i }}][image]" imageDesc="{{ $imageDesc3 }}">
